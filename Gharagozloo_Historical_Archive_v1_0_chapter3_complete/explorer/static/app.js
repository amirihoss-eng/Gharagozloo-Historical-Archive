...
async function curator(){
  ...
  try{
    ...
    app.innerHTML=`<section class="curator-shell"><div class="section-head"><div><div class=eyebrow>LOCAL ONLY · MANUAL EDIT</div><h2>Photo Curator</h2><p>Uploads and links are audit-recorded. Research-imported artifacts and links cannot be removed here.</p></div></div>
...
function route(name,arg){document.body.classList.toggle('graph-mode',name==='graphcore');if(name==='curator')return curator();if(name==='graphcore')return graphCore(arg);if(name==='home')return home();if(name==='people')return people(arg||'');if(name==='person')return person(arg);if(name==='timeline')return timeline();if(name==='gallery')return gallery();if(name==='estates')return estates();if(name==='estate')return estate(arg);if(name==='organizations')return organizations();if(name==='organization')return organization(arg);if(name==='titles')return titles();if(name==='title')return title(arg);if(name==='research')return research();}
